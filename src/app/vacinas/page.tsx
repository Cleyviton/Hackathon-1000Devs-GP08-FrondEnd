...
export default function App() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch("http://localhost:4000/consulta/vacina"); // Substitua pela URL da sua API
        const data = await response.json();
        console.log(data);
        setvacinas(data);
      } catch (error) {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {vacinas.map((vacina: any) => (
              ...
              >
                ...
                <p className="text-sm text-gray-500">dose: {vacina.dose}</p>
                <p className="text-sm text-gray-500">
                ...
                <p className="text-sm text-gray-500">
                  data_aplicacao:{" "}
                  {new Date(vacina.data_aplicacao).toLocaleDateString()}
                </p>
                <p className="text-sm text-gray-500">
